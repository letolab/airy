...
class HomeHandler(AiryHandler):
    ...
    def get(self):
        if not self.current_user:
            ...
            self.render("#menu-right", "user_out_menu.html")
            self.render("#content", "index.html", form=form)
            return
        user = self.get_current_user()
        self.render("#menu-right", "user_in_menu.html")
...
class AccountsRegisterHandler(AiryHandler):
    def get(self):
        form = RegistrationForm()
        self.render("#content", "accounts/register.html", form=form)
        self.execute('ubp.menu("register");')
...
class AccountsProfileHandler(AiryHandler):
    ...
    def get(self):
        user = self.get_current_user()
        self.render(
            "#content", "accounts/profile/main.html",
        )
        self.execute('ubp.menu("profile");')
...
class AccountsForeignProfileHandler(AiryHandler):
    def get(self, object_id):
        try:
            ...
            self.render(
                "#content", "accounts/profile/foreign_profile.html",
...
class FileUpload(AiryHandler):
    ...
    def get(self, action):
        if action == 'upload':
            ...
            self.render(
                "#profile-picture",
                "accounts/profile/picture_upload.html",
                form=form
            )
            self.execute('ubp.users.picture_init();')
        else:
            self.render("#profile-picture", "accounts/profile/picture.html")
...
class AccountsRecoverPasswordHandler(AiryHandler):
    def get(self, path=None):
        if not path:
            ...
            self.render("#content", "accounts/password_recovery/recovery.html", form=form)
            return

        args = urlparse.parse_qs(path)
        if not self.token_is_valid(args.get('token', [None])[0]):
            self.redirect("/accounts/recovery/")

        form = NewPasswordForm()
        self.render("#content", "accounts/password_recovery/new_password.html", form=form)
    ...
    def post(self, path=None):
        ...
        if not path:
            ...
            if form.is_valid():
                ...
                self.render("#content", "accounts/password_recovery/password_reset_sent.html")
            else:
                self.render("#content", "accounts/password_recovery/recovery.html", form=form)
            return
    ...
    def generate_recovery_link(self, user):
        ...
        return 'http://%s:%s/accounts/recovery/%s' % (HOST, PORT, params)
